...
from ..research.models import CrawlDecision, FetchedDocument, SearchResult
...
class Claim(BaseModel):
    """One atomic, checkable statement extracted from ONE source passage."""
    id: str
    category: str
    claim_type: ClaimType
    statement: str = Field(description="single sentence, self-contained, names the city/region explicitly")
    quote: str = Field(description="verbatim supporting text from the source, <= 60 words")
    source_url: str
    geo_level: GeoLevel = Field(description="geographic scope the evidence actually refers to")
    entities: list[str] = Field(default_factory=list, description="people, organisations, programmes, policies named")
    year: int | None = Field(default=None, description="year the fact refers to, if stated")
    extractor_confidence: float = Field(ge=0, le=1)


Verdict = Literal["SUPPORTED", "PARTIALLY_SUPPORTED", "CONFLICTING", "UNSUPPORTED", "MISSING"]


class Verification(BaseModel):
    claim_id: str
    verdict: Verdict
    rationale: str = Field(description="one or two sentences, cites what in the passage supports or fails the claim")
    geo_mismatch: bool = Field(default=False, description="true if the claim presents broader-level data as city data")
